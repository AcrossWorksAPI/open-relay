...
- merged experimental local watcher proof implementation
- merged experimental local relay watch implementation with bounded watch
  posting
...
| P1 | Implement local watcher proof | Done | No |
...
| P1 | Implement Orchestra session manager | Planned | No |
...
- Relay session identifiers are promoted into the planned Orchestra session
  manager slice: a random Open Relay-generated session id should visibly link
  participating thread titles, and a local session manifest should be added in
  the project/session orchestration layer without changing packet schemas.
