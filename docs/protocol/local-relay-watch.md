...
`review-response/0.1` packet, and posts or updates that packet on the same PR.
...
| `--interval-ms <ms>` | No | `30000` | Delay between foreground poll iterations when `--watch` is set. |
| `--output <receipt.json>` | No | stdout | Writes the latest machine-readable receipt. |
...
| `--no-update` | No | `false` | Posts a new response packet comment instead of updating the authenticated user's latest matching response packet comment. |

`--dry-run` cannot be combined with live confirmation flags.
...
12. Post or update the packet through the existing GitHub PR transport.
...
Each run writes a JSON receipt containing the PR target, author filter, mode,
status, state-file path, request identity, Claude prompt hash, optional Claude
session id, response summary, and failure reason when applicable.
