...
| Release workflow planning | In progress | PR #47 defines the recommended `@acrossworks/open-relay@0.1.0` first release gate, changelog/tag workflow, npm trusted publishing path, release preflight, no-live-claim closeout rules, committed `private: true` safety, and release-job-only private-field removal. |
...
Review PR #47 for release workflow planning, then implement the publish-ready
