...
| Product implementation | In progress | The validation slice is merged; packet generation from live git state remains planned. |
...
Draft the local git-state packet generator plan: inputs, redaction behavior,
packet storage location, and command shape for producing `review-request`
packets.
