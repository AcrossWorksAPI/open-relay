...
`--max-failures`, and writes local receipt/state evidence
...
| --- | --- | --- | --- |
| 2026-07-01 | Bounded live relay-watch failure proof | Failed safely | PR #60 / branch `codex/local-relay-watch` now adds `--max-failures`, default `1`, to stop live foreground polling after failed iterations. `npm run check` passed with 227 tests, `npm run smoke:pack` passed, `npm run release:preflight -- 0.1.0` passed, and `git diff --check` passed. Owner-approved R7M4Q9K2 live `--watch --max-posts 1 --max-failures 1 --confirm-live --confirm-public` fetched current review-request comment `4840707648` at head `3192e49361b025a2117982be441a1af13a4fa098`, invoked Claude, timed out before a valid review-response draft, wrote `/private/tmp/open-relay-r7m4q9k2-live-watch-receipt.000001.failed.json` with `error: "Claude review command timed out."`, exited at `--max-failures`, and posted no new PR response comment. |
