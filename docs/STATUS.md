...
| Local relay status indicator | In progress | Branch `codex/local-relay-status-indicator` adds optional `--status-file` and `--notify` support for the foreground relay watcher, including local status JSON, best-effort macOS notification warnings, parser/CLI/status tests, README/protocol docs, and package-smoke help coverage without daemon install, Codex wakeup, packet schema changes, fixes, merge, publish, or deploy. |
...
Finish and review the local relay status indicator branch, then run an
owner-approved relay-watch pass with `--status-file` and optional `--notify` so
the foreground orchestrator has visible local operator evidence. After that,
