...
| Local watcher proof implementation | Ready for review | PR #59 / branch `codex/local-watcher-proof` adds experimental `open-relay experimental watcher-proof`, Codex app-server trigger support, Claude headless trigger support, local secrets-env loading for Claude credentials, dry-run/live receipts, tests, package smoke coverage, and protocol docs without packet schema changes, GitHub posting, merge automation, publish, or daemon installation. |
