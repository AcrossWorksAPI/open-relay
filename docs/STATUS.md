...
Local relay watch implementation is in progress as the first foreground
...
`review-response/0.1`, posts or updates the response packet only with explicit
GitHub write confirmation, and writes local receipt/state evidence without
changing packet schemas, installing a daemon, waking Codex, applying fixes,
merging, publishing, or deploying.
...
| Local relay watch implementation | In progress | Branch `codex/local-relay-watch` adds experimental `open-relay experimental relay-watch` to fetch a PR review-request packet, render a Claude prompt, invoke headless Claude after `--confirm-live`, validate and post/update a review-response packet after `--confirm-public`, and maintain local receipt/state evidence without schema changes, daemon install, Codex wakeup, fixes, merge, publish, or deploy. |
...
| --- | --- | --- | --- |
| 2026-06-30 | Local relay watch implementation branch check | Passed | Branch `codex/local-relay-watch` adds `open-relay experimental relay-watch`, parser/state/receipt orchestration, dry-run fetch/render, fake-`gh` CLI dry-run coverage, injected live Claude-to-GitHub response posting coverage, README/protocol docs, package-smoke help coverage, and local relay-watch plan/governance updates without packet schema changes, daemon install, Codex wakeup, fixes, merge, publish, or deploy. `npm run check` passed with 225 tests, `npm run smoke:pack` passed, `npm run release:preflight -- 0.1.0` passed, and `git diff --check` passed. |
