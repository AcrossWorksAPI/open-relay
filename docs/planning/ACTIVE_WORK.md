...
invoking agents or applying fixes. Local watcher proof, local relay watch,
local relay status indicator, and local response watch are merged, proving the
...
applying fixes, merging, publishing, or deploying. The next planned slice is
the Orchestra session manager: promote Orchestra into the local user-facing
manager for current/previous sessions, automatic Relay Session ID generation,
session manifests, and no-turn Codex thread creation. A live proof before the
plan confirmed the Codex app-server supports `thread/start` plus
`thread/name/set` for creating and naming a fresh Codex thread without starting
an agent turn.
...
| `src/orchestraStatus.ts` | Active | Experimental local orchestra dashboard status builder, local HTTP server, static HTML renderer, and browser open helper. |
| `src/cli.ts` | Active | Local CLI entrypoint for packet validation, review-request generation/handoff/save, generic rendering, GitHub PR transport, experimental watcher proof routing, experimental relay watch routing, experimental response watch routing, and experimental orchestra dashboard routing. |
...
| `tests/orchestraStatus.test.ts` | Active | Orchestra dashboard parser, injected status snapshot, and HTML shell tests. |
...
| `docs/protocol/local-orchestra-dashboard.md` | Active | Experimental local orchestra dashboard command contract, status model, failure boundaries, and non-goals. |
...
| `docs/superpowers/plans/2026-07-02-local-orchestra-status-gui.md` | Active | Implementation plan for the passive local orchestra status dashboard and JSON snapshot. |
| `docs/superpowers/plans/2026-07-02-orchestra-session-manager.md` | Active | Implementation plan for promoting Orchestra into the local session manager with automatic Relay Session ID generation, session manifests, session listing, and no-turn Codex thread creation. |
...
| Relay session/thread identity absent | Low | Planned in `docs/superpowers/plans/2026-07-02-orchestra-session-manager.md`: Orchestra should generate random Relay Session IDs, write local session manifests, and create a linked Codex thread title such as `<id>-OR-CX`; packet schema fields remain deferred. |
...
1. Implement the Orchestra session manager plan so the user can launch
   Orchestra, create a new Open Relay session, get an automatic Relay Session
   ID, and open a ready Codex thread without seeing raw process logs.
2. Keep the existing foreground relay-watch and response-watch processes as
   the automation surfaces until the manager starts PR-specific loops after a
   PR exists.
3. Decide whether to publish `0.1.0` after the local automation/session-manager
   slice, or plan the remaining implementation-handoff packet type for
   `0.1.x`.
...
7. Keep Relay Session ID/thread-title linking in the project/session
   orchestration layer, not the packet schema.
