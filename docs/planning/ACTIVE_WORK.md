...
planning is in progress to add per-file churn evidence to request packets
...
| Packet evidence is thinner than brief | Medium | Diff summary capture is planned through packet evidence enrichment; test capture remains explicit `--verification` input rather than automatic command execution. |
...
1. Review PR #41's packet evidence enrichment planning package.
2. Implement packet evidence enrichment from
   `docs/superpowers/plans/2026-06-28-review-request-evidence-enrichment.md`.
3. Dogfood the merged loop on the packet evidence enrichment PR by sending a
