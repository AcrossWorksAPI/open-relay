...
| Repo-local packet storage planning | In progress | 2026-06-26 | Planning branch `codex/repo-local-packet-storage-planning` | PR #28: `https://github.com/AcrossWorksAPI/open-relay/pull/28` | N/A, docs/planning slice only | Design source `docs/superpowers/specs/2026-06-26-repo-local-packet-storage-design.md`; implementation source `docs/superpowers/plans/2026-06-26-repo-local-packet-storage.md`; `npm run check`, `npm run smoke:pack`, and `git diff --check` passed locally; Claude review finding F1 addressed by requiring manifest-last completion semantics and best-effort partial-bundle cleanup | Revert the planning branch if repo-local storage or `save review-request` is rejected. |
