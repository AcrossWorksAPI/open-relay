...
| Project foundation governance baseline | Done | 2026-06-26 | Branch `codex/project-foundation-baseline` | Draft PR #1: `https://github.com/AcrossWorksAPI/open-relay/pull/1` | N/A, docs-only baseline | Passed `git diff --check` on 2026-06-26 | Revert the baseline commits if owner rejects the workflow. |
| Open Relay brief and remote alignment | Done | 2026-06-26 | Branch `codex/project-foundation-baseline` | Draft PR #1: `https://github.com/AcrossWorksAPI/open-relay/pull/1` | N/A, docs-only baseline | Passed `git diff --cached --check` on 2026-06-26 | Revert the docs update commit if owner changes product direction. |
