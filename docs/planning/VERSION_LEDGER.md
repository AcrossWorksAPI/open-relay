...
| Runtime and schema CLI planning | In progress | 2026-06-26 | Branch `codex/runtime-schema-planning` | PR pending | N/A, docs/planning slice | Local governance checks passed on branch: `git diff --check`, roadmap parser, required files, placeholder scan, secret-like scan, JSON parse, and example parity check | Revert the planning PR if the TypeScript CLI-first direction changes. |
