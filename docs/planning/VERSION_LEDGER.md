...
| Repo-local packet storage implementation | In progress | 2026-06-27 | Branch `codex/repo-local-packet-storage-implementation` | PR pending | N/A, local CLI only; no global storage or external agent invocation | Adds `src/storage.ts`, `open-relay save review-request`, `.open-relay/` gitignore entry, CLI/storage tests, cleanup regression coverage, and installed-package save smoke; `npm run check` passed with 69 tests, `npm run smoke:pack` verified installed CLI save behavior, and `git diff --check` passed | Revert the implementation branch if saved bundle behavior or CLI command shape is rejected. |
