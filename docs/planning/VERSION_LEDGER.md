...
| Runtime/schema validation CLI implementation | In progress | 2026-06-26 | Branch `codex/runtime-schema-cli`; commits `16e9e5c`, `13a5483`, `f99959f`, `dcbc751` | PR pending | N/A, local CLI branch only | `npm ci`, `npm run check`, `git diff --check`, and `node dist/src/cli.js validate examples/review-request/relay.json` passed locally on 2026-06-26 | Revert the implementation PR if the validator or CLI direction is rejected. |
