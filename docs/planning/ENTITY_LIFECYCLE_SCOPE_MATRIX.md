...
| Packet transport | In progress | In progress | In progress | N/A | Deferred | N/A | Shipped | In progress | Shipped | Deferred | N/A | Shipped | GitHub PR exact-packet transport is in implementation with `send`, `fetch`, `--update`, base64 marker metadata, local `gh` permissions, author-filtered fetch, protocol docs, tests, and installed-package dry-run smoke; native GitHub review import, delete/archive, response storage, notifications, and automation remain deferred. |
