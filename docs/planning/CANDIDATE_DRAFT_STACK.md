...
| PC-002 Runtime and verification baseline | Choose TypeScript or Python and define local commands. | Product features and deployment. | PC-001 packet shape. | Add package/project config and verified commands. | No hidden stack assumptions; commands are real. | Run discovered commands plus `git diff --check`. |
